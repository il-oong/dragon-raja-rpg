...
## v1.0.15 (2026-04-19) — 페이싱 딜레이 + 스킬북 직업 필터

전반적으로 장면 전환이 너무 빨라 몰입이 깨지던 문제 해결 + 다른 직업 스킬북이 상점/서재에 뜨는 문제 해결.
...
- **장거리 이동 (6h+)** — 3초 같은 메시지
- `_busy` 상태 + `mkBtn` 가드로 연타 방지.
- 비용/시간 소모는 완료 시점에 처리 — 일관성.

### 스킬북 직업 호환성 필터 (PR #32)
- **증상**: 전사에게 마법사 스킬북이 상점에 노출되고, 사서 읽어도 전투 메뉴에 안 뜨는 문제.
- **원인**: `state.skills` 에는 들어가지만 전투 메뉴는 `state.jobs` 직업만 순회.
- **수정 5곳**:
  - `isBookCompatible(it)` 헬퍼 — `sourceJob ∈ state.jobs` 판정.
  - 상점: 비호환 책 숨김 (구매 불가).
  - 서재 `tryLibraryDiscovery`: 내 보유 직업 스킬만 후보.
  - 서재 미리보기: 내 직업 스킬만 예시 노출.
  - 마이페이지 인벤: 비호환 책 disabled + ⚠ + opacity 0.5.
  - engine cUse 가드: sourceJob ∉ state.jobs 면 명시적 거부.
