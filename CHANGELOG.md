...
## v1.0.26 (2026-04-21) — 세트 보스 드랍 + 잡동사니/암거래상 텍스트 드래곤 라자 톤

### 👑 세트당 1종은 보스 드랍 전용
각 컬렉션 세트를 완성(5/5 풀셋)하려면 해당 테마의 보스를 쓰러뜨려야 함.
암시장에서는 4/5 까지만 구할 수 있고 마지막 한 종이 보스 드랍.

| 세트 | 보스 전용품 | 드랍 보스 | 확률 |
|---|---|---|---|
| 🧙 마녀의 유물 | 찢어진 그리모어 | 흑사제장 칼릭스 (폐허된 성당) | 60% |
| 🐲 용 신봉자 | 용의 심장 조각 | 프로스트 드래곤 (Lv 40+) | 60% |
| ☠ 해적의 유산 | 해적 선장의 두개골 | 두목 지스카 (황금사슬 도적굴) | 60% |

- `bossOnly: true` 플래그 신규 — 암시장 진열 풀에서 자동 제외.
- 해당 보스 `drops` 배열에 추가.
- 마이페이지 💎 수집품 탭: 보스 전용 아이템은 **👑** 마크 + 미보유 시
  갈색 톤으로 표시 → 어디서 얻어야 할지 직관적.

### 📝 텍스트 드래곤 라자 톤 정리 (1차 패스)
- **잡동사니 16종 desc 재작성** — 짧고 건조하며 약간 비꼬는 서술체.
  예: "지도라고 하기엔 얼룩이 많고, 얼룩이라 하기엔 표식이 너무 정연하다."
- **암거래상 대사** — "세 시 좀 넘어서 찾아왔다면 당신은 올바른 시각에 온
  것이고, 그 전에 왔다면 — 잘못 온 것이지."
- 이번 릴리즈는 잡동사니·암거래상 중심. 나머지(저주장비·NPC·몬스터 설명 등)
  는 점진적 톤 조정 예정.

---

## v1.0.25 (2026-04-21) — 암시장 개장창 02:00~04:00 한정

### 🗝 암시장은 새벽 2시부터 4시까지 (2시간) 만 열림
- 기존 (v1.0.24): '밤' 시간대 전체 (20:00~04:59) 동안 암시장 접근 가능했음.
  밤이 너무 길어 긴장감 희석.
- 수정: 정확히 **02:00~04:00** 2시간만 개장. 새벽 깊은 시각 컨셉.
- `engine.BLACK_MARKET_HOURS = {from:2, to:4}` + `isBlackMarketHour()` 헬퍼.
  `shopOpen()` / `advanceTime()` (시간창 이탈 시 플래그 해제) 에 연동.
- 메인 메뉴: 02~04 시 안이면 '🗝 암시장' 버튼 활성, 그 외 밤 시간엔
  `🗝 암시장 (02~04시)` 로 비활성 + 툴팁으로 현재 시각 + 개장창 안내.

---

## v1.0.24 (2026-04-21) — 밤 = 상점 자리에 암시장, 희귀 스킬북 ×10 1종

### 🗝 밤에는 상점 버튼이 통째로 '🗝 암시장' 으로 전환
- 기존: 밤엔 '상점 (닫힘 🌙)' 으로 비활성. 암시장 열려면 talk 암거래상을 따로.
- 수정: **암거래상이 있는 지역 (헬탄트/수도/카밀카르)** 은 밤에 메인 메뉴의
  상점 버튼이 자동으로 **'🗝 암시장'** 으로 바뀌고, 클릭 시 `black_market` 플래그
  자동 세팅 + 바로 진입. 대화 스텝 생략.
- 낮/황혼엔 평소처럼 '상점' 버튼.

### 📘 희귀 스킬북 1종 (×10) 매일 랜덤 진열
- 기존 암시장: 고급/희귀/영웅 스킬북 3종 전부 ×3 프리미엄.
- 수정: 진열 구성 변경
  * **희귀/영웅/전설 스킬북 1종** (×10 프리미엄) — 핵심 상품. 매일 롤링.
  * **고급 스킬북 2종** (×3 프리미엄) — 서브.
  * 저주 장비 3종 · 잡동사니 3종 · 포션 2종 (그대로).
- `itemPrice()` 가 grade 체크 — 희귀 이상 스킬북만 ×10, 나머지는 ×3.

---

## v1.0.23 (2026-04-21) — 잡동사니 3세트 컬렉션 시너지

### 💎 도시별 잡동사니 세트 — 모을 수록 시너지
- 각 암시장 도시에 **고유 5종** 진열, 3/5종 이상 보유 시 티어 효과 활성.
- 공용 1종 (`junk_box` 봉인된 상자) 는 모든 암시장에 가끔 진열.

**세트 구성**
| 세트 | 도시 | 멤버 | 3종 티어 | 5종 티어 |
|---|---|---|---|---|
| 🧙 마녀의 유물 | 헬탄트 | 마른 손가락 · 유리 눈 · 붉은 머리카락 · 찢어진 그리모어 · 재가루 봉지 | INT+5 WIS+3 | INT+12 WIS+8, **마법력 ×1.10** |
| 🐲 용 신봉자 | 수도 | 정체불명 이빨 · 비늘 · 뿌러진 발톱 · 황금빛 눈알 · 작은 심장 표본 | STR+4 VIT+3 | STR+10 VIT+6, **용 타입 대미지 ×1.30** |
| ☠ 해적의 유산 | 카밀카르 | 보물 지도 · 럼주 빈 병 · 녹슨 열쇠 · 해적 두개골 · 검은 깃털 | DEX+3 LUK+4 | DEX+6 LUK+10, **골드 획득 ×1.20** |

- 마이페이지에 **💎 수집품** 탭 신규 — 세트별 보유 현황 + 활성 티어 효과
  + 합산 시너지 보여줌.
- 효과 배선: `Game.junkBonus()` 가 스탯(S)/마법력(getMag)/골드(combat 승리)/
  용 대미지(applyFoeDmg) 모두 반영.
- `amsijangCity` 필드로 각 도시 암시장 진열 필터링 — 세트 완성하려면 세 도시
  밤 방문 루프가 강제됨.

---

## v1.0.22 (2026-04-21) — 암시장 개편 + 일일 수련 한도

### 🗝 암시장 전용 재고 (저주/고급 스킬북/잡동사니 · 일일 로테이션)
- **기존**: 밤에 암거래상과 대화하면 `black_market` 플래그만 세워지고,
  정작 열어보면 그냥 낮 상점 = **[일반] 스킬북만 쭉** 나열. 이름값을 못 함.
- **신규 특수 재고** (암시장 진입 시):
  * **고급/희귀/영웅 스킬북 3개** — 내 **보유 직업 전체** (마스터 포함) 에 맞춰
    자동 필터. 새 직업 추가 시 그때부터 해당 직업 책도 진열됨.
  * **저주 장비 3개** — 내 직업 **계열(line)** 에 맞는 것만 진열.
  * **잡동사니 3개** — 효과 없음, 분위기용 (봉인된 상자 · 보물 지도 · 마녀의
    손가락 · 용의 비늘 등).
  * 고정: 고급 회복약 + 상급 마나약.
- **일일 로테이션** — `state.time.day + 지역` 시드로 매 게임일 진열 갱신.
- **×3 프리미엄** — 모든 상품 기본가의 3배. 저주/잡동사니/고급 스킬북은
  **암시장에서만** 구할 수 있음.

### 💀 저주 장비 10종 신규 (+ 기존 3종 flag 부여)
모두 `cursed: true` + `line` 태그 보유 → 직업 계열 필터 대응.
- 전사: 피의 검, 영혼 포식 반지, 울부짖는 판금, 데몬 소드·군주
- 마도: 파멸의 목걸이, 리치의 왕관, 광기의 관, 저주받은 지팡이
- 궁수: 피의 장궁
- 도적: 맹독 단검, 그림자 망토
- 성직/바드: 역십자 성물, 울음의 묵주, 세이렌의 류트

### 🎲 잡동사니 10종 (암시장 감성)
정체불명의 이빨 · 구겨진 보물 지도 · 봉인된 상자 · 마른 손가락 · 유리로 된 눈 ·
정체불명의 비늘 · 빈 병 · 녹슨 열쇠 · 작은 두개골 · 검은 깃털.

### ⏰ 하루 수련 시간 제한 (실제 시간 90분/day)
- 같은 실제 날짜 동안 누적 수련 실시간이 **90분을 넘으면** 수련 시작 거부.
- 남은 시간보다 긴 수련을 요청하면 남은 시간만큼 자동 축소 (gameDays 도 비례 축소).
- `state.trainedDayKey` (YYYY-MM-DD) + `state.trainedRealSec` 로 영속화.
  날짜가 바뀌면 자동 리셋.
- 수련장 UI 에 **오늘의 수련 한도 진행바 + 남은 시간** 실시간 표시.

### 🛠 기타
- `engine.shopInventory()` / `engine.itemPrice()` 공용 헬퍼 추가 — 엔진 텍스트
  출력 / renderShop 버튼 가격이 한 경로에서 일치.
- 일반 상점에서도 스킬북은 보유 직업 적합만 노출 (기존엔 텍스트 덤프엔 전부 나왔음).

---

## v1.0.21 (2026-04-20) — 게임 화면 테마 + 창 불투명도 (몰래 플레이 강화)

### 🎨 게임 화면 자체에도 테마 적용
- 기존: 테마 (sysmon/excel/terminal/kakao) 는 **위장 화면만** 바꿨고 게임 화면은
  항상 어두운 다크 톤. "몰래 플레이" 컨셉에서 어두운 창이 오히려 튐.
- 수정: `setTheme()` 이 `body[data-game-theme]` 속성도 함께 세팅 →
  `#output / #side / #actions / #topbar / button` 모두 같은 계열로 재색칠.
  * **sysmon** (기본): 현재 다크 톤 유지
  * **excel**: 흰 배경 + 녹색 포인트 (스프레드시트 룩)
  * **terminal**: 검정 + 초록 (로그 tail 룩)
  * **kakao**: 파랑 배경 + 노랑 버튼 (카톡 채팅방 룩)

### 🫥 창 불투명도 슬라이더 (0.3 ~ 1.0)
- 마이페이지 🔖 버전 탭 에 **불투명도 슬라이더** 추가 (프리셋 100/80/50% 버튼).
- 렌더러에서 `set-window-opacity` IPC → main.js 가 `win.setOpacity()` 호출.
- localStorage 에 영속화 — 앱 재시작해도 이전 값 복원.
- 창 뒤가 살짝 비쳐 일하는 척 더 자연스러움. 진짜 작업창 보며 게임 가능.

---

## v1.0.20 (2026-04-20) — 로그인 시 클라우드 자동 동기화

### ☁ 내 아이디로 로그인하면 내 캐릭터가 뜨도록
- **문제 (사용자 제보)**: 다른 PC 에서 내 아이디로 로그인해도 그 PC 의 **로컬
  캐릭터 목록이 그대로 노출** — 로그인은 토큰만 받고 세이브는 여전히
  `saves.json` 에서만 읽었기 때문. "남의 캐릭터가 로드되네" 사태.
- **수정**: `autoLoadCloudOnLogin()` 헬퍼 추가 — 로그인/가입 성공 직후
  `/api/cloud/load` 호출.
  * 클라우드에 내 세이브 있음 → 로컬 덮어쓰고 불러옴 (로컬 비어있으면 조용히,
    뭐라도 있으면 `N개 삭제 후 덮어씀` 확인 다이얼로그)
  * 클라우드 비어있고 로컬에 있음 → 업로드 권유
- **저장 시 자동 푸시**: `saveGame()` 이 로그인 상태면 `/api/cloud/save` 를
  백그라운드로 호출 → 저장 = 클라우드 최신화. 푸시 실패는 경고만 남기고 로컬
  저장 성공은 그대로 유지.
...
- 기존: 로그(`#output`) / 선택지(`#actions`) 영역이 `2fr : 3fr` 비율로 고정돼
  있어 긴 서술을 읽을 때 스크롤이 잦고 답답했음.
- 두 영역 사이에 **드래그 가능한 스플리터** 추가 — 세로로 자유롭게 리사이즈.
  최소 80px 보호(두 영역 모두), 창 리사이즈 시 자동 재-클램프.
- **로컬 저장**: 조정한 높이를 `localStorage['dr_layout_output_h']` 에 픽셀 단위로
  저장 → 앱 재시작 후에도 유지.
- **더블클릭으로 리셋**: 스플리터 바를 더블클릭하면 기본 비율(`2fr:3fr`)로 복귀.

### 🕓 NPC 등장 시간대 ↔ 퀘스트 시간대 정합성 수정 (데이터 레이어)
v1.0.17 의 UX 가드(버튼 비활성 + 툴팁) 는 시간대 불일치를 **알려주는** 수정
이었고, 이번 버전은 **원인 자체를 없애는** 데이터 교정:
- **상인 실바나** (엘프 마을) — `band: ['낮','황혼']` → `['새벽','낮','황혼']`.
  자신이 주는 `q_dawn_market`("새벽 시장 경비", `timeBand:['새벽']`) 수주 가능해짐.
- **대주교 유스티스** (수도) — `band: ['새벽','낮','황혼']` → `['새벽','낮','황혼','밤']`.
  메인 체인 `q_main_4`("대주교의 시험", `timeBand:['밤']`) 수주·완료 가능해짐.
  밤 철야 기도 컨셉과도 부합.
- 결과적으로 v1.0.17 의 `🔒 시간대` 라벨이 실제로 뜨는 상황은 드물어짐 —
  두 레이어가 맞물려 동작.

### 🗝 암거래상 대사 맥락 교정
- 밤에만 등장하는 NPC인데 기존 대사가 `"내 물건은 낮엔 팔지 않아"` 라 **밤에
  찾아온 플레이어 입장에서 낮을 언급하는 어색한 표현**이었음 (사용자 제보).
- 새 대사: `"...조용히. 어둠 아래서만 거래한다. 동이 트기 전까지만 내 물건을 볼 수 있어."`
  — 현재 시간대(밤)와 맞고, 동틀 때 암시장 닫히는 동작도 자연스럽게 암시.

---

## v1.0.17 (2026-04-20) — 맞춤 수련 + 테마 순환 + 시간대 퀘스트 UX

### 🧘 맞춤 수련 시간 (1~60분, 레벨 제한 없음)
- 기존: 5분/15분/30분/60분/120분 프리셋만 존재, Lv.10 미만은 5분 "가벼운 명상"
  만 사용 가능해 저레벨 체감이 답답했음.
- 수련장에 **맞춤 수련 입력란** 추가 — 1~60분 사이 자유 선택, 레벨 제한 없음.
  게임 일수는 `max(1, round(min/3))` 로 비례 계산, XP 예상치 실시간 미리보기.
- 프리셋은 그대로 유지 (빠른 선택용).
- 엔진 `startFocus(min, days, { custom: true })` 경로로 레벨 게이트 우회 + 60분 clamp.

### 🐛 위장 테마 순환 안 됨 — 버그 수정
- `cycleTheme()` 가 `DISGUISE_THEMES` 배열 대신 data.js 의 **지역 테마 dict
  (`THEMES`)** 를 참조하고 있었음. `THEMES.indexOf(...)` 는 undefined → `NaN`
  인덱스 → `setTheme(undefined)` → 'sysmon' 폴백 → **Ctrl+Shift+T 누를 때마다
  sysmon 으로 리셋** 되는 증상.
- `DISGUISE_THEMES.indexOf(...)` 로 교정. 4종 순환 정상 동작.

### 🐛 시간대 한정 퀘스트 UX — 수락 버튼은 뜨는데 누르면 실패
- q_main_4 (밤), q_midnight (밤), q_dawn_market (새벽) 은 각자 timeBand
  제약이 있는데 **NPC 는 그 이전 시간대에 이미 나타나서** 버튼만 뜨고
  클릭 시 "X 시간대에만 수주 가능" 경고로 거부되던 문제. (황혼에 상인 실바나
  에게서 q_dawn_market 보이는데 수락 안되고 "새벽에 와" 하는 상황 등.)
- `renderNpc()` 퀘스트/완료 버튼에 현재 시간대 체크 추가 —
  조건 불일치 시 `🔒 이름 🕓허용시간대` 로 비활성화 표시. 어느 시간대에 와야
  하는지 라벨과 툴팁 양쪽에서 안내.

---

## v1.0.16 (2026-04-20) — 핫픽스: 로그인 화면 무반응

### 🐛 Critical
- v1.0.15 에서 `const act = document.getElementById('actions')` 가 페이싱 딜레이
  블록으로 교체되며 **통째로 누락**. 이 때문에 `render()` 첫 줄 `act.innerHTML = ''`
  에서 `ReferenceError: act is not defined` 가 튀어 메인 영역이 렌더 안 됨.
  로그인 타이틀만 찍히고 폼/버튼이 전혀 안 보이던 증상의 원인.
- `act` 레퍼런스 복구 — v1.0.15 로 설치한 사용자는 반드시 업데이트 필요.

---

## v1.0.15 (2026-04-19) — 페이싱 딜레이

전반적으로 장면 전환이 너무 빨라 몰입이 깨지던 문제 해결.

### 추가된 딜레이
- **서재 찾기** — 10초 + 5단계 플레이버 메시지
  ("서가를 훑는다" / "먼지 쌓인 책등을 살핀다" / "고대어 목차 해독" /
   "관심 장을 펼친다" / "깊은 구절에 집중")
- **탐험** — 1.5초 "🔍 주변을 둘러본다..."
- **단거리 이동 (5h 이하)** — 1.5초 "🚶 <지역> 쪽으로 이동 중..."
- **장거리 이동 (6h+)** — 3초 같은 메시지

### 구현
- `_busy` 상태 + `mkBtn` 가드 — 딜레이 중 버튼 클릭 모두 무시 (연타 방지).
- `withDelay(ms, msg, cb)` 단일 메시지 헬퍼.
- `withProgress(totalMs, steps, progressFn, cb)` 분할 진행 헬퍼.
- 비용/시간 소모는 **완료 시점에** 처리 — 도중 사용자가 창을 닫아도 상태 일관성 유지.

---

## v1.0.14 (2026-04-19) — PR #28 통합 + QA 3 에이전트 버그 수정

### 🎮 컨트리뷰터 PR #28 통합 (uowol)
+1043 / -82. 시간대·NPC·퀘스트·보스페이즈·단축키 대규모 추가:
- 4구간 시간대 (새벽/낮/황혼/밤) + 자동 전환 로그
- NPC_DIALOG 9명 트리 + 종족 조건부 선택지
- 상점/스킬북/수련장 시간 게이트 · 암거래상 밤 한정
- 야간 전용 몬스터 12종 · 지역별 encountersByBand
- 사냥의 날(Day%7==0) + 엘리트 추적
- 시간 한정 보스 3종 (달빛 늑대왕 / 여명의 불새 / 자정의 리치)
- 9개 보스에 2~3페이즈 (아무르타트 3단계)
- 메인 퀘스트 8스텝 체인 + 서브 10 · 반복 5 · 시한 2 → **5 → 32 퀘스트**
- 전투 단축키 (1/2/3 행동 · q/w/e 스킬 · a/s/d 아이템)
- 여관 숙박 시간 선택 (1h / 8h / dawn)
- 사이드 패널 달력 카드 + 시간대 화면 틴트
- 드워프-게롤트 유대 → 영구 5% 상점 할인

### 📚 서재를 수련장과 분리
- 기존: 수련장 수련 종료 시 확률로 스킬북 발견 (XP 와 혼용)
- 현재: 새 uiMode `library` — 메인메뉴 [📚 서재] 버튼
  * 세션당 100G + 게임 시간 2h 소비 → tryLibraryDiscovery
  * 밤엔 서재 닫힘 (시간 게이트)
  * 발견 가능한 책 미리보기 (미습득 중 랜덤 5개)

### 🐛 QA 에이전트 3개 발견 버그 수정
**보스 페이즈 (QA #1)**: 치명타/원샷으로 여러 임계를 한번에 넘기거나 사살될 때
페이즈 메시지가 누락되던 문제. `if (f.hp <= 0) return;` 조기 종료 제거.
힐/공격력 버프는 살아있을 때만 적용 (시체 회복 방지).

**퀘스트 체인 (QA #2)**: 메인 `q_main_2~8` 이 NPC 대화에 노출 안돼 접근 불가능
였던 문제. `completeQuest` 에서 `q.next` 가 있으면 `state.quests[next]` 를
자동으로 생성 → 체인 자연스럽게 이어짐. `q_meir_son_2` 조기 수락 우회도 부수적 차단.

**guardian_angel (QA #3)**: `avatar` 직업의 guardian_angel effect 가 persist
whitelist 에 누락되어 per-combat 3-charge 로만 동작하던 문제. `angel` 키워드
화이트리스트 추가.

### 🔢 수치 안정성 가드
- completeQuest: reward.gold/exp 누락 또는 NaN 시 0 처리.
- 전투 승리: 배율 체인 NaN → 0 으로 정리 후 state 에 반영.
- checkLevel: state.exp NaN 리셋 + MAX_LV 200 상한 → 무한루프 방지.

### 🧪 QA 하네스
- `scripts/qa-harness.js` — Node 에서 engine.js 직접 구동.
- 7 시나리오 (smoke / combat / quest / passive / time / library / boss) 전부 green.
