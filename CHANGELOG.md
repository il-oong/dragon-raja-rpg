...
## v1.0.35 (2026-05-19) — 픽셀 그래픽 + 전투 스킬 모션

### 🎨 픽셀 그래픽
- 캐릭터, 몬스터, 아이템, 지역/마을 장면을 코드 기반 픽셀 SVG로 렌더링.
- 전투 화면, 메인 지역 패널, 인벤토리, 사이드 캐릭터 카드에 픽셀 아트를 연결.

### ⚔ 전투 액션 모션
- 일반 공격, 물리 스킬, 마법 스킬, 회복, 버프, 디버프, 아이템, 도주별 애니메이션 추가.
- 스킬 타입에 따라 베기, 투사체, 회복 파동, 버프/디버프 오라, 피격 흔들림을 자동 분류.

---

## v1.0.32 (2026-04-28) — 친구 시스템 UI & 게임플레이 개선

### 🎮 게임플레이 개선
- **탐험 딜레이 제거** - 탐험 버튼 즉시 실행 (전투/스킬만 딜레이 유지)
- **몬스터 조우 확률 대폭 조정** - 2마리 이상 조우 확률 39% → 70% 증가
  - 1마리: 60% → 29%
  - 2마리: 25% → 40%
  - 3마리: 10% → 20%
  - 무리(4-6마리): 4% → 10%
...
### 👥 친구 시스템 UI 추가
- **친구 목록** - 친구 확인, 메시지 보내기, 친구 삭제
- **친구 요청** - 받은 요청 수락/거절, 새 친구 요청 보내기
- **메시지함** - 대화 목록, 읽지 않은 메시지 배지
- **실시간 채팅** - 친구와 1:1 메시지 주고받기
- 캐릭터 선택 화면에서 👥 친구 버튼으로 접근
